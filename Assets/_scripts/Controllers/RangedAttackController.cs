...
public class RangedAttackController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            FingerHitSystem hitSystem = collision.GetComponent<FingerHitSystem>();
            if(hitSystem != null)
            {
                hitSystem.ChangeHealth(-_attackData.power);
                if (_attackData.isOnKnockback)
                {
                    DefaultMovement movement = collision.GetComponent<DefaultMovement>();
                    if(movement != null)
                    {
